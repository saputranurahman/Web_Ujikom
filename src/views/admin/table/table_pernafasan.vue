            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="text-gray-500">
                        <tr v-for="pernafasan in getDataPernafasan" :key="pernafasan.id">
                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                <p class="whitespace-no-wrap">{{ pernafasan.id }}</p>
                            </td>
                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                <p class="whitespace-no-wrap">{{ pernafasan.video }}</p>
                            </td>
                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                <p class="whitespace-no-wrap">{{ pernafasan.catatan }} </p>
                            </td>
                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                <p class="whitespace-no-wrap">{{ pernafasan.TingkatStre.kategori }} </p>
                            </td>
                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                ...
                                <div class="flex space-x-4">
                                    <button class="flex items-center text-blue-500 hover:text-blue-700">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="2" stroke="currentColor" class="h-6 w-6 mr-1">
                                            <path stroke-linecap="round" stroke-linejoin="round"
...
export default {
    ...
    computed: {
        ...mapGetters('pernafasan', ['getDataPernafasan'])
    },
    methods: {
        ...mapActions('pernafasan', ['fetchDataPernafasan']),
        openModal() {
            this.modalOpen = true;
        },
        closeModal() {
