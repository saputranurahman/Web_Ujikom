...
const saran = {
  ...
  getters: {
    getDataSaran: (state) => state.dataSaran,
  },
  actions: {
    async fetchDataSaran({ commit }) {
        try {
          ...
          const data = await axios.get("http://localhost:8080/api/v1/saran", config);
          commit("SET_SARAN_DATA", data.data['data']);
        } catch (error) {
          console.error(error);
          throw error;
        }
  ...
  mutations: {
    SET_SARAN_DATA(state, dataSaran) {
      state.dataSaran = dataSaran;
    },
